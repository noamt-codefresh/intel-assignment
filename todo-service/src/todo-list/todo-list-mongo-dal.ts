...
import Q = require("q");
import {TODO_LIST_DB_NAME} from "../types/data-store-types";
...
export class TodoListMongoDal implements TodoListDal {
    ...
    public async init(mongoClient: MongoClient): Promise<void> {
        if (!mongoClient.isConnected()) {
            return Q.reject<void>(new Error("failed to initialize TodoListMongoDal, no available mongo connection"));
        }
